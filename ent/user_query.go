...
func (uq *UserQuery) QueryMembership() *MembershipQuery {
...
func (uq *UserQuery) WithMembership(opts ...func(*MembershipQuery)) *UserQuery {
	query := (&MembershipClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withGroup; query != nil {
		if err := uq.loadGroup(ctx, query, nodes,
	...
	if query := uq.withMembership; query != nil {
		if err := uq.loadMembership(ctx, query, nodes,
			func(n *User) { n.Edges.Membership = []*Membership{} },
			func(n *User, e *Membership) { n.Edges.Membership = append(n.Edges.Membership, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadGroup(ctx context.Context, query *GroupQuery, nodes []*User, init func(*User), assign func(*User, *Group)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(user.GroupPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.GroupPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.GroupPrimaryKey[1]))
...
func (uq *UserQuery) loadMembership(ctx context.Context, query *MembershipQuery, nodes []*User, init func(*User), assign func(*User, *Membership)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Membership(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.MembershipColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
