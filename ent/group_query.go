...
func (gq *GroupQuery) QueryMembership() *MembershipQuery {
	query := (&MembershipClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withMembership:      gq.withMembership.Clone(),
...
func (gq *GroupQuery) WithMembership(opts ...func(*MembershipQuery)) *GroupQuery {
	query := (&MembershipClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withMembership = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withMembership; query != nil {
		if err := gq.loadMembership(ctx, query, nodes,
			func(n *Group) { n.Edges.Membership = []*Membership{} },
			func(n *Group, e *Membership) { n.Edges.Membership = append(n.Edges.Membership, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadMembership(ctx context.Context, query *MembershipQuery, nodes []*Group, init func(*Group), assign func(*Group, *Membership)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(membership.FieldGroupID)
	}
	query.Where(predicate.Membership(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.MembershipColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
